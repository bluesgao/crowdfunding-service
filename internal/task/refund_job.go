...
// NewRefundJob 创建项目退款任务
func NewRefundJob(db *gorm.DB, cfg *config.Config, chainManager *chain.Manager) *RefundJob {
	return &RefundJob{
		db:           db,
		config:       cfg,
...
// GetName 获取任务名称
func (j *RefundJob) GetName() string {
	return "project_refund_updater"
}

// GetSchedule 获取调度配置
func (j *RefundJob) GetSchedule() gocron.JobDefinition {
	return gocron.DurationJob(time.Duration(j.config.Task.Interval) * time.Second)
}

// Execute 执行任务
func (j *RefundJob) Execute() {
...
func (j *RefundJob) processRefund(record model.RefundRecordModel, project model.ProjectModel, contributeRecord model.ContributeRecordModel) error {
...
func (j *RefundJob) callContractRefund(project model.ProjectModel, record model.RefundRecordModel) error {
...
func (j *RefundJob) updateRefundStatus(recordId int64, status string, errorMsg string) {
	updates := map[string]interface{}{
		"status": status,
	}
