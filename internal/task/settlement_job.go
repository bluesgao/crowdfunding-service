...
// NewSettlementJob 创建项目结算任务
func NewSettlementJob(db *gorm.DB, cfg *config.Config, chainManager *chain.Manager) *SettlementJob {
...
// GetName 获取任务名称
func (j *SettlementJob) GetName() string {
	return "project_settlement_updater"
}

// GetSchedule 获取调度配置
func (j *SettlementJob) GetSchedule() gocron.JobDefinition {
	return gocron.DurationJob(time.Duration(j.config.Task.Interval) * time.Second)
}

// Execute 执行任务
func (j *SettlementJob) Execute() {
...
func (j *SettlementJob) processSettlement(record model.SettlementRecordModel, project model.ProjectModel) error {
...
func (j *SettlementJob) callContractSettlement(project model.ProjectModel, amount int64) error {
...
func (j *SettlementJob) recordPlatformFee(projectId int64, fee int64) error {
...
func (j *SettlementJob) updateSettlementStatus(recordId int64, status string, errorMsg string) {
	updates := map[string]interface{}{
		"status": status,
	}
