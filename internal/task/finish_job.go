...
// NewFinishJob 创建项目完成任务
func NewFinishJob(db *gorm.DB, cfg *config.Config, chainManager *chain.Manager) *FinishJob {
	return &FinishJob{
		db:           db,
		config:       cfg,
...
// GetName 获取任务名称
func (j *FinishJob) GetName() string {
	return "project_finish_updater"
}

// GetSchedule 获取调度配置
func (j *FinishJob) GetSchedule() gocron.JobDefinition {
	return gocron.DurationJob(time.Duration(j.config.Task.Interval) * time.Second)
}

// Execute 执行任务
func (j *FinishJob) Execute() {
...
func (j *FinishJob) handleSuccessfulProject(project model.ProjectModel) {
...
func (j *FinishJob) handleFailedProject(project model.ProjectModel) {
