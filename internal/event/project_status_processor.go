...
// ProjectStatusProcessor 项目状态事件处理器
type ProjectStatusProcessor struct {
	projectLogic *logic.ProjectLogic
}

// NewProjectStatusProcessor 创建项目状态事件处理器
func NewProjectStatusProcessor(projectLogic *logic.ProjectLogic) *ProjectStatusProcessor {
	return &ProjectStatusProcessor{
		projectLogic: projectLogic,
	}
}

// Process 处理项目状态事件
func (p *ProjectStatusProcessor) Process(event *model.Event, eventData map[string]interface{}) error {
	// 获取项目状态
	status := eventData["status"].(uint64)
